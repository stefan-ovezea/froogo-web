...
export function FilterChips({
  ...
  return (
    <div className="flex w-full items-center gap-2 overflow-x-auto px-4 py-2 scrollbar-hide">
      {onClear && selectedOptions.size > 0 && (
        <button
          onClick={onClear}
          className="flex flex-shrink-0 items-center gap-1 rounded-xl bg-surface-container-high px-4 py-2 text-sm font-medium text-on-surface transition-colors hover:bg-surface-container-highest"
        >
      ...
      {options.map((option, index) => {
        ...
        return (
          ...
          >
            ...
            <span className="relative z-10">{option}</span> {/* Added span for text to be above gradient */}
          </button>
        );
      })}
    </div>
